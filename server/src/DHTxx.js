    ...
    this.samplingPromise = new Promise((resolve, reject) => {
      ...
      const handler = (e, t, h) => {
        ...
        if (h < RANGE[this.type].rhmin || h > RANGE[this.type].rhmax)
          sample.humidity_dubious = `${Math.trunc(h)}% out of range ${RANGE[this.type].rhmin}%..${RANGE[this.type].rhmax}%, enter it manually`;
        if (t < RANGE[this.type].tmin || t > RANGE[this.type].tmax)
          sample.temperature_dubious = `${Math.trunc(t)}C out of range ${RANGE[this.type].tmin}C..${RANGE[this.type].tmax}C, enter it manually`;
